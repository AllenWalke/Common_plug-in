...
ScrollPic.prototype = {
	...
	initialize: function() {
		...
		if (this.dotListId) {
			...
			if (this.dotListObj && !!!this.dotListObj.innerHTML) {
				for (i = 0; i < pages; i++) {
				...
				this.dotObjArr = this.getEleByClassName("dot_list",this.dotListObj,"li");
				var length = this.dotObjArr.length;
				if(length){
					for (i = 0; i < length; i++) {
						(function(index){
							tempObj = thisTemp.dotObjArr[index];
							if (index == thisTemp.pageIndex) {
								tempObj.className = thisTemp.dotOnClassName;								
							} else {
								tempObj.className = thisTemp.dotClassName;
							};
							tempObj[thisTemp.listEvent] = function() {
								thisTemp.pageTo(index);
							}
						})(i);
					}
				}
	...
	getEleByClassName:function(className,element,tag) {
