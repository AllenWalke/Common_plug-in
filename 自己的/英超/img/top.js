;(function(win, doc) {
    ...
    var DropDown = (function() {
        ...
        var showTeamClz = navClz + '-shown';
        var timerId = null;
        var show = function() {
            clearTimeout(timerId);
            removeClass(dom.nav, showTeamClz);
        ...
        return {
            init: function() {
                ...
                addEvent(dom.teamLnk, 'mouseenter', showTeam);
                addEvent(dom.teamLnk, 'mouseleave', hide);
                addEvent(dom.teamSub, 'mouseenter', showTeam);
                addEvent(dom.teamSub, 'mouseleave', hide);

                addEvent(dom.guideLnk, clickType, show);
                addEvent(dom.teamLnk, clickType, showTeam);
            ...
            init: function(w, config) {
                ...
                UserPanel = UserPanel.set('outLoginLayer', {
                    'ready' : function() {
                        ...
                        if (LoginLayer) {
                            LoginLayer.set('sso', {
                                entry: config.entry || 'account'          // 设置entry
                            }).set('styles', {
                                // 'margin-top': '0px',
                                // 'z-index': '10001'
                            }).set('plugin', {
                                ...
                                isFn(config.login_success, info);         // 设置login_success函数
                                ...
                                isFn(config.logout_success, info);         // 设置logout_success函数
                            }).init();
                        }
                    }
                });
